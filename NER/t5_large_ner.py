...
wandb.init(project="Dravida Ulagam - Large", name='t5large-ner-dec2-run1', config={
    "model": "t5-base",
    "batch_size": 8,
    "learning_rate": 2e-5,
    "epochs": 5,
...
def tokenize_fn(batch):
    ...
    targets = tokenizer(batch["target_text"], padding="max_length", truncation=True, max_length=512)
...
def parse_ner_output(output, max_len=512):
...
def compute_metrics(eval_pred):
    ...
    parsed_preds = [parse_ner_output(pred, max_len=512) for pred in decoded_preds]
    parsed_labels = [parse_ner_output(label, max_len=512) for label in decoded_labels]
    ...
    # Log metrics to WandB through trainer.log
    metrics = {
        "precision": precision,
        "recall": recall,
        "f1": f1,
    ...
    trainer.log(metrics)  # Logs directly to WandB
...
training_args = Seq2SeqTrainingArguments(
    output_dir="/scratch/ds7395/nlp_proj/t5_large_ner_run1",
...
trainer.log_metrics("train", {"convergence_epoch": convergence_epoch if convergence_epoch is not None else None})
wandb.finish()
